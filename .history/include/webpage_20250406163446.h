...
<body>
    <div class="container">
        ...
        <div class="action-group-controls">
            <h3 class="action-group-title">动作组控制</h3>
            <button class="action-btn record-btn" onclick="sendActionCommand('action_record_start')">开始记录</button>
            <button class="action-btn stop-btn" onclick="sendActionCommand('action_record_stop')">停止记录</button>
            <button class="action-btn play-btn" onclick="sendActionCommand('action_play')">播放动作</button>
        ...
        function onMessage(event) {
            try {
                ...
                if (data.hasOwnProperty('actionStatus')) {
                    document.getElementById('action-status').innerHTML = data.actionStatus;
                }
            } catch (error) {
