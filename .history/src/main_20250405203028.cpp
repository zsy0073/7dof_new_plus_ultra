...
void initDisplayAndSensor() {
  ...
  for (int size = 0; size <= 64; size += 4) {
    display.clearDisplay();
    display.drawRect(0, 0, size, size, WHITE); // 修正动画绘制逻辑
    display.display(); // 确保每次更新后显示内容
    delay(100);
  }
...
void initI2C() {
  ...
  Wire.setClock(400000); // 设置 I2C 时钟频率为 400kHz
  delay(100); // 等待 I2C 初始化完成
}

//用于显示温度和学生信息的任务函数
void displayAndTemperatureTask(void *parameter) {
  while (1) {
    float ambientTemp = mlx.readAmbientTempC(); // 读取实时温度
    displayMessage("学号: " + String(studentID), 
                   "姓名: " + String(studentName), 
                   "温度: " + String(ambientTemp) + " C"); // 调整显示形式
