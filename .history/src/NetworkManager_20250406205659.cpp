  ...
  if (command == "R") {
    ...
    if (colonPos != -1 && colonPos < command.length()-1) {
      ...
      for (int i = 0; i < idStr.length(); i++) {
        if (!isDigit(idStr.charAt(i)) && idStr.charAt(i) != '-') {
          isValidID = false;
          break;
        }
      }
      ...
      for (int i = 0; i < posStr.length(); i++) {
        if (!isDigit(posStr.charAt(i)) && posStr.charAt(i) != '-') {
          isValidPos = false;
          break;
        }
      }
      ...
      if (isValidID && isValidPos) {
        ...
        if (cmdType == "J" && id >= 0 && id < 7) {
          ...
          Serial.println("WebSocket: 无效的命令类型");
        }
      } else {
        Serial.println("WebSocket: ID或位置值不是有效数字");
      }
    } else {
