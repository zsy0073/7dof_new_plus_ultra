          ...
          if (colonPos != -1 && colonPos < command.length() - 1) {
            ...
            if (isValidID && isValidPosition) {
              ...
              if (cmdType == "J" && id >= 0 && id < 7) {
                ...
                Serial.print("无效的命令类型或ID: ");
                Serial.print(cmdType);
                Serial.print(", ID=");
                Serial.println(id);
              }
            } else {
              if (!isValidID) Serial.println("ID不是有效数字");
              if (!isValidPosition) Serial.println("位置值不是有效数字");
            }
          } else {
            Serial.println("命令格式错误: 缺少分隔符或格式不正确");
          }
        }
