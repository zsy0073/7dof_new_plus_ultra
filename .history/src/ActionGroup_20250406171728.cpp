...
// 播放动作组中的一个动作
void playOneAction(ServoAction& action) {
  // 检查是否是复位命令
  if (action.servoId == RESET_ACTION_ID) {
    Serial.print("播放动作: [复位所有舵机], 时间=");
  ...
  // 执行常规动作
  moveServo(action.servoId, action.position, action.moveTime);
...
bool updatePlayActions() {
  ...
  if (millis() >= nextActionTime) {
    ...
    // 更新下一个动作的时间
    nextActionTime = millis() + 100; // 允许较短间隔播放下一个动作
