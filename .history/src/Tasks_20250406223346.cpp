...
void servoControlTask(void *parameter) {
  ...
  while (1) {
    ...
    if (!isCommandExecuting && !isPlaying && xQueueReceive(servoCommandQueue, &cmd, 0) == pdTRUE) {
      ...
      // 等待当前命令执行完成
      // 这里可能需要等待一段时间，取决于舵机移动时间
      // moveServo函数会更新lastMoveEndTime
    }
...
void ps2ControllerTask(void *parameter) {
  // 初始化PS2控制器
  bool ps2Initialized = initPS2Controller();
  
  if (!ps2Initialized) {
    // 如果初始化失败，等待一段时间后重试
    vTaskDelay(pdMS_TO_TICKS(5000));
    ps2Initialized = initPS2Controller();
    
    // 如果还是失败，打印警告并继续运行(可能稍后会插入手柄)
    if (!ps2Initialized) {
      Serial.println("警告: PS2手柄初始化失败，请检查连接");
    }
  }
  ...
  while (1) {
    ...
    // 不需要额外的延迟，handlePS2Input函数内已有延迟
  }
}
