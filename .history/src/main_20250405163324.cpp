...
void parseCommand(String command) {
  if (command == "R") {
    // 复位所有舵机
    for (int i = 0; i < 7; i++) {
      servos.moveServo(jointServos[i], 90, 1000); // 中心位置调整为 90
      armStatus.joints[i] = 90;
    }
    servos.moveServo(gripperServo, 90, 1000); // 中心位置调整为 90
...
void handleControllerCommand() {
  static String commandBuffer = ""; // 用于存储接收到的命令

  while (Serial1.available() > 0) {
    char receivedChar = Serial1.read(); // 读取一个字符
    if (receivedChar == '*') { // 结束符，表示命令接收完成
      Serial.print("Received command: "); // 添加调试信息
      ...
      parseCommand(commandBuffer); // 解析命令
      commandBuffer = ""; // 清空缓冲区

      // 处理完成后关闭LED
      digitalWrite(LED_PIN, LOW);
    } else {
      commandBuffer += receivedChar; // 将字符添加到缓冲区
    }
  }
}
