...
void initDisplayAndSensor() {
  mlx.begin(0x5A); // 设置 MLX90614 的 I2C 地址为 0x5A
  if (!mlx.begin()) {
    LOG_ERROR("Error connecting to MLX90614 at address 0x5A");
    while (1);
  }

  if (!display.begin(SSD1306_SWITCHCAPVCC, 0x3C)) { // 设置 OLED 的 I2C 地址为 0x3C
...
void initI2C() {
  Wire.begin(I2C_SDA_PIN, I2C_SCL_PIN); // 初始化I2C接口
  LOG_INFO("I2C initialized on GPIO6 (SDA) and GPIO7 (SCL)");
}
