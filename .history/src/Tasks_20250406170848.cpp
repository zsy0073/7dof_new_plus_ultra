...
bool areAnyServosMoving() {
    // Implement logic to check if any servo is moving
    // For example, return false if no servos are moving
    return false;
}
  ...
  while (1) {
    ...
    if (!isCommandExecuting && !isPlaying && xQueueReceive(servoCommandQueue, &cmd, 0) == pdTRUE) {
      ...
      if (cmd.isReset) {
        ...
        Serial.println("]");
    if (isCommandExecuting && !areAnyServosMoving() && millis() >= servoLastMoveEndTime) {
        // Placeholder for areAnyServosMoving function
    }
        // 处理普通舵机控制命令
