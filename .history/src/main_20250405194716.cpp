...
void initI2C() {
  ...
  LOG_INFO("I2C initialized on GPIO13 (SDA) and GPIO14 (SCL)");
}

// 修改 I2C 设备扫描函数，添加调试信息
void scanI2CDevices() {
  LOG_INFO("Starting I2C device scan...");
...
void displayAndTemperatureTask(void *parameter) {
  ...
  while (1) {
    if (showTemperature) {
      float ambientTemp = mlx.readAmbientTempC(); // 采集温度
      displayMessage("Ambient Temp: " + String(ambientTemp) + " C", 
                     "ID: " + String(studentID), 
                     "Name: " + String(studentName)); // 显示温度
    } else {
      displayMessage("ID: " + String(studentID), "Name: " + String(studentName)); // 显示学号和姓名
    }
    showTemperature = !showTemperature; // 切换显示内容
    vTaskDelay(pdMS_TO_TICKS(2000)); // 每2秒更新一次显示
  }
}
...
void setup() {
  ...
  initI2C(); // 初始化I2C接口
  LOG_INFO("Performing I2C device scan...");
