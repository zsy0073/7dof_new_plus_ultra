...
#include <queue.h> // Include FreeRTOS queue header

// 实例化全局对象
PS2Controller ps2Controller;
QueueHandle_t servoCommandQueue; // Declare the queue globally

PS2Controller::PS2Controller() {
    servoCommandQueue = xQueueCreate(10, sizeof(ServoCommand)); // Initialize the queue
    if (servoCommandQueue == NULL) {
        Serial.println("Failed to create servoCommandQueue");
    }
PS2Controller::PS2Controller() {
    isConnected = false;
    lastUpdateTime = 0;
